  ...
  return (
    <section ref={sectionRef} className="px-1">
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <h3 className="text-4xl md:text-5xl lg:text-6xl font-black tracking-[0.06em] uppercase leading-[0.95] bg-gradient-to-r from-[#0f766e] via-[#1f2937] to-[#0f766e] bg-clip-text text-transparent drop-shadow-sm">
          VISIT US ON YOUTUBE
        </h3>
        <Link href="https://www.youtube.com/" target="_blank" className="text-[#0f766e] font-bold hover:underline">
          View All
        </Link>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {displayVideos.map((video, index) => (
          <div key={`${video.videoId}-${video.title}-${index}`} className="group">
                ...
                <>
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="w-20 h-20 rounded-full bg-white flex items-center justify-center shadow-xl">
                      <Play className="w-10 h-10 text-[#0f766e] ml-1" fill="currentColor" />
                    </span>
                  </div>
                </>
              )}
            ...
            <p className="font-semibold text-slate-900 mt-4 mb-3 text-3xl leading-tight">{video.title}</p>

            <Link
              href="/contact"
              className="inline-flex items-center justify-center bg-[#0f766e] text-white px-6 py-2.5 rounded-md font-bold hover:bg-[#115e59] transition"
            >
              Get Quote
            </Link>
          </div>
